...
import { getAnosFinaisFiltrosDisponiveis, getAnosFinaisResumoPorCiclo } from "@/lib/queries/anos-finais";
import { ANOS_FINAIS_TURMA_SENTINELA_ESCOLA } from "@/lib/anos-finais-catalogo";
...
import { HabilidadeGrid } from "../caed/habilidade-grid";

/**
 * Painel "Anos Finais" — espelha `app/admin/avaliacoes/caed/page.tsx` (mesma
 * plataforma técnica CAEd/UFJF, mesma taxonomia de 3 níveis), mas página e
 * fonte de dados própria: nunca mistura com o painel do Criança Alfabetizada
 * (anos iniciais). Só escolas municipais aparecem aqui — o pareamento por
 * código INEP contra a tabela `Escola` (sincronizada via SIGEduc, só rede
 * municipal) já exclui escolas estaduais automaticamente na importação.
 */
...
export default async function PainelAnosFinaisPage({ searchParams }: PageProps) {
  const opcoes = await getAnosFinaisFiltrosDisponiveis();

  if (opcoes.etapas.length === 0 || opcoes.componentes.length === 0) {
    return (
      <div>
        <PageHeader
          title="Painel Anos Finais"
          description="Avaliação Contínua da Aprendizagem nos Anos Finais — 6º ao 9º ano (MEC/CAEd)"
        />
        <EmptyState
          className="mt-6"
          icon={ClipboardCheck}
          title="Nenhum dado dos Anos Finais importado ainda"
          description="Rode scripts/extrair-anosfinais-escolas.ts, depois mapear e importar, pra ver participação, desempenho e acerto por habilidade aqui."
        />
      </div>
    );
  }
  ...
  const ciclos = await getAnosFinaisResumoPorCiclo({ etapaEnsino: etapaSelecionada, componenteSlug: componenteSelecionado });
  ...
  if (ciclos.length === 0) {
    return (
      <div>
        <PageHeader title="Painel Anos Finais" description="Avaliação Contínua da Aprendizagem nos Anos Finais (MEC/CAEd)" />
        {filtros}
        <EmptyState
          className="mt-6"
          icon={ClipboardCheck}
  ...
  function tabHref(tabId: string) {
    ...
    return `/admin/avaliacoes/anos-finais?${params.toString()}`;
  }
    ...
    <div>
      ...
      <div className="mt-8">
        ...
        <p className="mt-2 text-xs text-foreground-muted/70">
          {ciclosDoAno.some((c) => c.resumo.mediaAdequadoPonderada !== null)
            ? "Ponderado pelo número de estudantes avaliados de cada escola quando esse dado existe; cai para média simples entre escolas nas linhas sem essa contagem."
      ...
      <div className="mt-8">
        ...
        <p className="mt-1 text-xs text-foreground-muted/70">
          Todos os ciclos já importados para {etapaLabel} · {componenteLabel} — o próprio portal só mostra o ciclo
          selecionado; aqui dá pra comparar ao longo do tempo. {TEXTO_TRANSPARENCIA_EVOLUCAO_ANUAL}
        </p>
      ...
      <div className="mt-8">
                ...
                {paginaEscolas.map((l) => (
                  <TableRow key={`${l.escolaId}:${l.turma}`}>
                    <TableCell className="font-medium text-foreground">
                      {l.escolaNome}
                      {l.turma !== ANOS_FINAIS_TURMA_SENTINELA_ESCOLA && <span className="text-foreground-muted"> — {l.turma}</span>}
                    </TableCell>
        ...
        <Pagination
          ...
          basePath="/admin/avaliacoes/anos-finais"
